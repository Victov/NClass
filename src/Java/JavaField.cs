...
namespace NClass.Java
{
    ...
    {
        private const string ModifiersPattern = @"((?<modifier>static|final|volatile)\s+)*";
        private const string InitValuePattern = @"(?<initvalue>[^\s;](.*[^\s;])?)";

        // [<access>] [<modifiers>] <type> <name> [= <initvalue>]
        private const string FieldPattern = @"^\s*" + JavaLanguage.AccessPattern + ModifiersPattern + @"(?<type>" + JavaLanguage.GenericTypePattern2 + @")\s+" + @"(?<name>" + JavaLanguage.NamePattern + @")" + @"(\s*=\s*" + InitValuePattern + ")?" + JavaLanguage.DeclarationEnding;
