...
namespace NClass.CSharp
{
    ...
    {
        private const string AccessorAccessPattern = @"(protected\s+internal\s+|" + @"internal\s+protected\s+|internal\s+|protected\s+|private\s+)";

        // [<access>] [<modifiers>] <type> <name>["["<args>"]"]
        // ["{"] [[<getaccess>] get[;]] [[<setaccess>] set[;]] ["}"]
        private const string PropertyPattern = @"^\s*" + CSharpLanguage.AccessPattern + CSharpLanguage.OperationModifiersPattern + @"(?<type>" + CSharpLanguage.GenericTypePattern2 + @")\s+" + @"((?<name>" + CSharpLanguage.GenericOperationNamePattern + @")|(?<name>this)\s*\[(?<args>.+)\])" + @"\s*{?\s*(?<get>(?<getaccess>" + AccessorAccessPattern + @")?get(\s*;|\s|$))?" + @"\s*(?<set>(?<setaccess>" + AccessorAccessPattern + @")?set(\s*;)?)?\s*}?" + CSharpLanguage.DeclarationEnding;

        private const string PropertyNamePattern = @"^\s*(?<name>" + CSharpLanguage.GenericOperationNamePattern + @")\s*$";
