...
namespace NClass.CSharp
{
    internal sealed class CSharpEvent : Event
    {
        ...
        private const string EventPattern = @"^\s*" + CSharpLanguage.AccessPattern + CSharpLanguage.OperationModifiersPattern + @"event\s+" + @"(?<type>" + CSharpLanguage.GenericTypePattern2 + @")\s+" + @"(?<name>" + CSharpLanguage.GenericOperationNamePattern + ")" + CSharpLanguage.DeclarationEnding;

        private const string EventNamePattern = @"^\s*(?<name>" + CSharpLanguage.GenericOperationNamePattern + @")\s*$";
