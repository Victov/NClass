...
namespace NReflect
{
    ...
    internal class ReflectionWorker
    {
        ...
        /// </summary>
        private const BindingFlags STANDARD_BINDING_FLAGS = BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance;
        ...
        {
            NRAssembly nrAssembly = new NRAssembly {FullName = assembly.FullName, Source = "<Memory>"};
        ...
        private void ReflectModules( IEnumerable< Module > modules, NRAssembly nrAssembly )
        {
            ...
            {
                NRModule nrModule = new NRModule {Name = module.Name};
        ...
        private void ReflectDelegate( Type type, IEntityContainer entityContainer )
        {
            ...
            else
            {
                ...
                nrDelegate = new NRDelegate {ReturnType = GetTypeUsage( methodInfo.ReturnType, methodInfo )};
        ...
        private void ReflectEnum( Type type, IEntityContainer entityContainer )
        {
            ...
            {
                nrEnum = new NREnum {UnderlyingType = type.GetEnumUnderlyingType( ).FullName};
                ...
                foreach ( FieldInfo field in fields )
                {
                    ...
                    if ( field.Name == "value__" )
                        continue;
                    NREnumValue nrEnumValue = new NREnumValue {Name = field.Name};
        ...
        /// <param name="nrGenericType">
        ///     An instance of <see cref="NRGenericType" /> which will
        ///     receive the reflected results.
        /// </param>
        ...
        private void ReflectEvents( Type type, NRCompositeType nrCompositeType )
        {
            ...
            foreach ( EventInfo eventInfo in eventInfos )
            {
                ...
                NREvent nrEvent = new NREvent {Name = eventInfo.Name, Type = GetTypeUsage( eventInfo.EventHandlerType, eventInfo ), TypeFullName = eventInfo.EventHandlerType.FullName ?? eventInfo.EventHandlerType.Name};
        ...
        private void ReflectConstructors( Type type, NRSingleInheritanceType nrSingleInheritanceType )
        {
            ...
            {
                NRConstructor nrConstructor = new NRConstructor {Name = typeName, AccessModifier = GetMethodAccessModifier( constructorInfo ), OperationModifier = GetOperationModifier( constructorInfo )};
        ...
        private void ReflectParameters( MethodBase methodBase, List< NRParameter > nrParameters )
        {
            ...
            {
                NRParameter nrParameter = new NRParameter {Name = parameter.Name, Type = GetTypeUsage( parameter.ParameterType, parameter, methodBase.DeclaringType ), TypeFullName = parameter.ParameterType.FullName ?? parameter.ParameterType.Name, ParameterModifier = ParameterModifier.In};
        ...
        /// <param name="attributable">
        ///     An instance of <see cref="IAttributable" />
        ///     to store the reflected attributes to.
        /// </param>
        ...
        {
            NRField nrField = new NRField {Name = fieldInfo.Name, AccessModifier = GetFieldAccessModifier( fieldInfo ), IsReadonly = fieldInfo.IsInitOnly, IsStatic = fieldInfo.IsStatic, Type = GetTypeUsage( fieldInfo.FieldType, fieldInfo ), TypeFullName = fieldInfo.FieldType.FullName ?? fieldInfo.FieldType.Name};
        ...
        {
            NRMethod nrMethod = new NRMethod {Name = methodInfo.Name};
        ...
        /// <param name="singleInheritanceType">
        ///     The reflected operator will be added to this <see cref="NRSingleInheritanceType" />
        ///     .
        /// </param>
        ...
        /// <param name="methodContainer">
        ///     This will be used to determine if the method is part of an
        ///     interface or not. Nothing will be added to this container.
        /// </param>
        ...
        /// </summary>
        /// <param name="type">The type which will be represented by the resulting <see cref="NRTypeUsage" />.</param>
        /// <param name="customAttributeDatas">
        ///     An <see cref="IEnumerable{T}" /> with attribute of th type which is used to
        ///     determine if the type is dynamic.
        /// </param>
        /// <param name="declaringType">A <see cref="Type" /> which defines the type which is used.</param>
        /// <returns>The initialized <see cref="NRTypeUsage" />.</returns>
        private static NRTypeUsage GetTypeUsage( Type type, IEnumerable< CustomAttributeData > customAttributeDatas, Type declaringType )
        {
            ...
            if ( dynamicAttributeData != null )
                ...
                {
                    ReadOnlyCollection< CustomAttributeTypedArgument > arguments = ( ReadOnlyCollection< CustomAttributeTypedArgument > ) dynamicAttributeData.ConstructorArguments[ 0 ].Value;
        ...
        /// <returns>The initialized <see cref="NRTypeUsage" />.</returns>
        private static NRTypeUsage GetTypeUsage( Type type, ref List< bool > dynamicAttributeData, Type currentType, List< Type > genericArguments )
        {
            ...
            NRTypeUsage nrTypeUsage = new NRTypeUsage {Name = GetTypeName( type ), Namespace = type.IsGenericParameter ? null : type.Namespace, IsNullable = nullable, IsDynamic = isDynamic, DeclaringType = declaringTypeUsage, FullName = type.FullName};
        ...
        private static string GetTypeName( Type type )
        {
            ...
            else if ( !type.IsGenericType && type.IsByRef && ( type.GetGenericArguments( ).Length > 0 ) && ( type.FullName != null ) && type.FullName.StartsWith( "System.Nullable`1" ) && type.FullName.EndsWith( "&" ) )
            {
            ...
            else if ( type.GetGenericArguments( ).Length > 0 )
            {
                if ( typeName.ToString( ).LastIndexOf( '`' ) > 0 )
                    typeName.Remove( typeName.ToString( ).LastIndexOf( '`' ), typeName.Length - typeName.ToString( ).LastIndexOf( '`' ) );
            }
        ...
        {
            EventInfo[] eventInfos = type.GetEvents( STANDARD_BINDING_FLAGS );
            List< string > eventNames = new List< string >( from eventInfo in eventInfos where eventInfo.DeclaringType == type select eventInfo.Name );
            return eventNames;
        }
        ...
        private IEnumerable< NRAttribute > GetAttributes( IEnumerable< CustomAttributeData > attributeDatas )
        {
            ...
            foreach ( CustomAttributeData attributeData in attributeDatas )
            {
                ...
                if ( ( attributeType != null ) && ( ( attributeType == typeof( DynamicAttribute ) ) || ( attributeType == typeof( ExtensionAttribute ) ) || ( attributeType == typeof( OutAttribute ) ) || ( attributeType == typeof( ParamArrayAttribute ) ) || ( attributeType == typeof( OptionalAttribute ) ) ) )
                    continue;
                NRAttribute nrAttribute = new NRAttribute {Name = GetTypeName( attributeType ), Namespace = attributeType != null ? attributeType.Namespace : ""};
        ...
        {
            NRAttributeValue nrAttributeValue = new NRAttributeValue {Type = argument.ArgumentType.FullName, Value = argument.Value};
        ...
        private IEnumerable< NRTypeParameter > GetTypeParameters( IEnumerable< Type > genericArguments )
        {
            ...
            foreach ( Type genericArgument in genericArguments )
            {
                ...
                NRTypeParameter nrTypeParameter = new NRTypeParameter {Name = genericArgument.Name, IsStruct = ( attributes & GenericParameterAttributes.NotNullableValueTypeConstraint ) != 0, IsClass = ( attributes & GenericParameterAttributes.ReferenceTypeConstraint ) != 0, IsIn = ( attributes & GenericParameterAttributes.Contravariant ) != 0, IsOut = ( attributes & GenericParameterAttributes.Covariant ) > 0};
                nrTypeParameter.IsConstructor = ( ( attributes & GenericParameterAttributes.DefaultConstructorConstraint ) != 0 ) && !nrTypeParameter.IsStruct;
        ...
        private static void ChangeOperationModifierIfOverwritten( Type type, MethodBase method, NROperation nrOperation )
        {
            if ( ( type != null ) && IsMethodOverwritten( type.BaseType, method ) )
                if ( method.IsVirtual && ( ( method.Attributes & MethodAttributes.VtableLayoutMask ) != MethodAttributes.VtableLayoutMask ) )
                {
                    if ( method.IsFinal )
