...
namespace NReflect.NRCode
{
    ...
    public static class CSharp
    {
        ...
        /// </summary>
        private static readonly Dictionary< string, string > EscapeChars = new Dictionary< string, string > {{"\\", @"\\"}, {"\"", "\\\""}, {"\'", "\\\'"}, {"\a", @"\a"}, {"\b", @"\b"}, {"\f", @"\f"}, {"\n", @"\n"}, {"\r", @"\r"}, {"\t", @"\t"}, {"\v", @"\v"}, {"\0", @"\0"}};

        /// <summary>
        ///     A mapping of type names to replace for output. For example: The
        ///     type System.Int32 should be printed as int.
        /// </summary>
        private static readonly Dictionary< string, string > TypeReplacement = new Dictionary< string, string > {{typeof( bool ).FullName, "bool"}, {typeof( byte ).FullName, "byte"}, {typeof( sbyte ).FullName, "sbyte"}, {typeof( char ).FullName, "char"}, {typeof( decimal ).FullName, "decimal"}, {typeof( double ).FullName, "double"}, {typeof( float ).FullName, "float"}, {typeof( int ).FullName, "int"}, {typeof( uint ).FullName, "uint"}, {typeof( long ).FullName, "long"}, {typeof( ulong ).FullName, "ulong"}, {typeof( object ).FullName, "object"}, {typeof( short ).FullName, "short"}, {typeof( ushort ).FullName, "ushort"}, {typeof( string ).FullName, "string"}, {typeof( void ).FullName, "void"}};
