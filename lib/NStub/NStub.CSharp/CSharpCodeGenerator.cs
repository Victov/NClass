...
namespace NStub.CSharp
{
    ...
    public class CSharpCodeGenerator : ICodeGenerator
    {
        ...
        ///     cannot be found.
        /// </exception>
        public CSharpCodeGenerator( CodeNamespace codeNamespace, string outputDirectory )
        {
            #region Validation
        ...
        public void GenerateCode( )
        {
            ...
            foreach ( CodeTypeDeclaration codeTypeDeclaration in CodeNamespace.Types )
            {
                ...
                CodeNamespace codeNamespace = new CodeNamespace( CodeNamespace.Name );

                // Clean the type name
                codeTypeDeclaration.Name = Utility.ScrubPathOfIllegalCharacters( codeTypeDeclaration.Name );

                // Create our test type
                codeTypeDeclaration.Name = Utility.GetUnqualifiedTypeName( codeTypeDeclaration.Name ) + "Test";
        ...
        /// </summary>
        /// <value>
        ///     The <see cref="System.CodeDom.CodeNamespace" /> object the
        ///     generator is currently working from.
        /// </value>
        ...
        ///     The <see cref="CodeTypeDeclaration" />
        ///     from which to remove the duplicates.
        /// </param>
        private static void RemoveDuplicatedMembers( CodeTypeDeclaration codeTypeDeclaration )
        {
            for ( int i = 0; i < codeTypeDeclaration.Members.Count; ++i )
            {
                ...
                for ( int j = 0; j < codeTypeDeclaration.Members.Count; ++j )
                    if ( codeTypeDeclaration.Members[ i ].Name.Equals( codeTypeDeclaration.Members[ j ].Name ) )
                    {
                        occurrences++;
                        if ( occurrences > 1 )
        ...
        private static void CreateStubForCodeMemberMethod( CodeMemberMethod codeMemberMethod )
        {
            ...
            codeMemberMethod.CustomAttributes.Add( new CodeAttributeDeclaration( new CodeTypeReference( typeof( TestAttribute ) ) ) );
            codeMemberMethod.CustomAttributes.Add( new CodeAttributeDeclaration( new CodeTypeReference( typeof( IgnoreAttribute ) ) ) );
            codeMemberMethod.Statements.Add( new CodeCommentStatement( "TODO: Implement unit test for " + codeMemberMethod.Name ) );
        }
